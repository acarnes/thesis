...
\begin{equation}
\delta S = 0 = \int m\ddot{x} \epsilon dt
\end{equation}

And this equation must be zero for any small deviation $\epsilon$.

\begin{equation}
\delta S = 0 \rightarrow m\ddot{x} = 0
\end{equation}

So a particle free of forces will keep the same velocity. Note that a Newtonian boost by constant velocity $v \rightarrow v' = v + u$ \footnote{Renaming $\dot{x}$ as v.} leaves the equations of motion consistent. In the unprimed frame the particle has velocity v. In the primed frame the particle has velocity v + u. In the unprimed frame the particle has 0 acceleration and the same is true in the primed frame. Both observers see the particle act as if there are zero forces.
...
\begin{equation}
\delta F = m\frac{d}{dt}(v+u) = m\dot{v} + m\dot{u} - m\dot{v} = m\dot{u}
\end{equation}

In the unprimed frame, the particle identified by the mass moves with constant velocity, $\dot{v} = 0$. The observer in the primed frame looks at the particle with the same mass and sees it change velocity given by the equation $\dot{v} = -\dot{u}$. As an example, set v and $u, \dot{u}$ to zero for all times before t=0, and let $u, \dot{u}$ turn on after time 0. Both observers will agree that the particle is stationary up until time 0. After which, the observer in the primed frame will see the particle accelerate in strange ways and the unprimed frame will continue to observe a stationary particle. 

If the unprimed frame tries to derive the laws of physics... 
If the observer in the primed frame tries to derive the laws of physics ... 

Every inertial frame finds $\delta F = 0$ and every accelerating frame finds an extra force $\delta F$ unique to its acceleration. In this way it appears that the laws of nature are symmetric in inertial reference frames. 

Just by opening ones eyes and performing an experiment it's impossible to tell which frame 
